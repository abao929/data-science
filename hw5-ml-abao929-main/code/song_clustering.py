...
def cluster_songs(music_data, max_iters=300):
    ...
    for k in range(1, MAX_CLUSTERS+1):
        kmeans = Kmeans(music_data, k, max_iters)
        centroids, idx = kmeans.run()
        inertias.append(kmeans.inertia(centroids, idx))
    elbow_point_plot(np.array(range(1, MAX_CLUSTERS+1)), np.array(inertias))
    K = 4

    # TODO: Run k-means with your chosen K
    centroids, idx = Kmeans(music_data, K, max_iters).run()

    # TODO: Use the visualize_songs_clusters() function to plot clusters
    visualize_songs_clusters(music_data, centroids, idx, False)

    # TODO: Visually compare the output from your Kmeans object to sk_learn_cluster() (print clusters and plot the song clusters)
    centroids_sklearn, idx_sklearn = sk_learn_cluster(music_data, K)
    visualize_songs_clusters(music_data, centroids_sklearn, idx_sklearn, True)
